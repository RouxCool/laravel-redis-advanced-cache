...
class RedisCacheService
{
    ...
    private function initRedis(): void
    {
        if (!$this->enabled) {
            if ($this->debug) {
                \Log::info('[RedisCacheService] Redis cache is disabled.');
            }
        ...
        try {
            ...
            if ($this->debug) {
                \Log::info('[RedisCacheService] Redis connection established successfully.');
            }
        } catch (\Throwable $e) {
            $this->redis = null;
            if ($this->debug) {
                \Log::error('[RedisCacheService] Redis connection failed: '.$e->getMessage());
            }
        }
    }
        ...
        try {
            ...
            do {
                ...
                if (!empty($results)) {
                    ...
                    if ($this->debug) {
                        \Log::info('[RedisCacheService] Deleted cache keys: '.implode(', ', $results));
                    }
                }
            ...
            $this->redis = null;
            if ($this->debug) {
                \Log::error('[RedisCacheService] Error deleting keys with pattern '.$pattern.': '.$e->getMessage());
            }
        }
    }
        ...
        try {
            ...
            do {
                ...
                if (!empty($keys)) {
                    $this->redis->del($keys);
                    if ($this->debug) {
                        \Log::info('[RedisCacheService] Flushed cache keys: '.implode(', ', $keys));
                    }
                }
            } while ($cursor !== 0 && $cursor !== null);
        } catch (\Throwable $e) {
            $this->redis = null;
            if ($this->debug) {
                \Log::error('[RedisCacheService] Error flushing Redis cache: '.$e->getMessage());
            }
        }
    }
    ...
    public function listenToWriteQueries(): void
    {
        if (!$this->enabled || !$this->redis || !$this->listenEnabled) {
            if ($this->debug) {
                \Log::info('[RedisCacheService] Write query listening is disabled.');
            }
        ...
        try {
            DB::listen(function ($query) {
                if ($operation = RedisCacheUtils::detectWriteOperation($query->sql)) {
                    ...
                    foreach ($flushables as $flushKey) {
                        ...
                        if ($this->debug) {
                            \Log::info('[RedisCacheService] Cache invalidated for table: '.$flushKey.' (SQL: '.$query->sql.')');
                        }
                    }
                }
            });
            ...
            if ($this->debug) {
                \Log::info('[RedisCacheService] Listening to database write queries for cache invalidation.');
            }
        } catch (\Throwable $e) {
            $this->redis = null;
            if ($this->debug) {
                \Log::error('[RedisCacheService] Error listening to write queries: '.$e->getMessage());
            }
        }
    }
}
